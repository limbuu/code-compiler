package com.spring.codecompiler.language.python;
...
public class PythonService {
	...
	public Map<String,String> solvePythonChallenge(CodeChallenge codeChallenege, String sourceCode) throws IOException {
		
		Map<String, String> output = new HashMap<>();
		File rootDirectory = this.fileManager.createDirectory(FileServiceConstants.basic_file_Path);
		File file = this.fileManager.createFile(rootDirectory+FileServiceConstants.python_file_name);
		boolean writeStatus = this.fileManager.writeDataOnFile(file, sourceCode);
		
		String[] runCommand = {"python", file.getAbsolutePath()}; 
		if (writeStatus) {
